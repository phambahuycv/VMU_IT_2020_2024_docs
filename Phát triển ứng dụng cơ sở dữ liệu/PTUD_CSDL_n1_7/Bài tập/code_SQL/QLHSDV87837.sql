...
--cau 2
Go
alter function  fuChibo(@ma char(5))
returns table
as 
	return(
	select MaCB,TenCB,TenDB,SoDangVien 
	from ChiBo cb inner join DangBo db on cb.MaDB= db.MaDB
	where db.MaDB=@ma
	)

Go
	select * from fuChibo('db001')
	select * from DangBo,ChiBo
Go
-- cau 3
create trigger capnhat
on ChiBo for insert, update, delete
as
begin
update DangBo set TongSoChiBo = TongSoChiBo+1
from DangBo inner join inserted on DangBo.MaDB= inserted.MaDB
update DangBo set TongSoChiBo = TongSoChiBo-1
from DangBo inner join deleted on DangBo.MaDB= deleted.MaDB
end
Go